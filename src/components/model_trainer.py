...
from src.components.data_ingestion import TrajectoryDataset, TrajectoryDatasetExcavation, ValidationTrajectoryDataset
from src.utils import mpjpe_error, euclidean_transform_3D, get_random_walk_noise_for_position_sequence
...
pos_train = np.load('/home/jasimusmani/mlpmixer_ssgp-main/dataset/pos_train_excav_20.npy')
force_train = np.load('/home/jasimusmani/mlpmixer_ssgp-main/dataset/force_train_excav_20.npy')
pos_valid = np.load('/home/jasimusmani/mlpmixer_ssgp-main/dataset/pos_valid_excav_20.npy')
force_valid = np.load('/home/jasimusmani/mlpmixer_ssgp-main/dataset/force_valid_excav_20.npy')
pos_test = np.load('/home/jasimusmani/mlpmixer_ssgp-main/dataset/pos_test_excav_20.npy')
force_test = np.load('/home/jasimusmani/mlpmixer_ssgp-main/dataset/force_test_excav_20.npy')
        ...
        for batch_idx, (train, gt) in enumerate(train_dataloader):
            ...
            for i in range(iteration):
                ...
                if sim == 'wheel':
                    ...
                    # loss1 = criterion(prediction[:16,:], gt_data[:16,:])
                ...
                if sim == 'excavation':
                    ...
                    gt_data = gt_data.to(device)
                    prediction = model(train_data)
                    # prediction = prediction.reshape(29, 3)
                    # gt_data = gt_data.reshape(29, 3)
                    prediction = prediction.reshape(9, 3)
                    gt_data = gt_data[:,:,20:29,:]
                    gt_data = gt_data.reshape(9, 3)
                    # print('prediction tensor shape',prediction.shape)
            ...
            for batch_idx_valid, (valid, gt_valid) in enumerate(valid_dataloader):
                ...
                if sim == 'excavation':
                    ...
                    for k in range(iteration_valid):
                        ...
                        gt_data = gt_data.to(device)
                        prediction_valid = model(valid_data)
                        # prediction_valid = prediction_valid.reshape(29, 3)
                        # gt_data = gt_data.reshape(29, 3)
                        prediction_valid = prediction_valid.reshape(9, 3)
                        gt_data = gt_data[:,:,20:29,:]
                        gt_data = gt_data.reshape(9, 3)
                        loss = mpjpe_error(prediction_valid, gt_data)
        ...
        for batch_idx, (test, _) in enumerate(test_dataloader):
            ...
            if sim == 'excavation':
                ...
                for i in range(306):
                    ...
                    gt_test_ =gt_test_.reshape(1,20,3)
                    prediction_test = model(initial_data)
                    # prediction_test = prediction_test.reshape(1, 29, 3)
                    prediction_test = prediction_test.reshape(1, 9, 3)
                    prediction_test = torch.cat([gt_test_, prediction_test], dim=1)
                    rollout_prediction.append(prediction_test)
